...
import { positionLocal, time, sin, cos, abs, pow, mix, clamp, smoothstep, float, vec3, color, mx_worley_noise_float, mx_fractal_noise_float } from 'three/tsl'
import type { StatusEffectId } from '~/stores/game'

type OverlayEffectId = 'poisoned' | 'burning' | 'frozen'
...
const OVERLAY_EFFECTS: Record<OverlayEffectId, EffectConfig> = {
...
const PRIORITY: OverlayEffectId[] = ['burning', 'poisoned', 'frozen']

/** Worley-based flowing overlay — used for poisoned */
function buildWorleyEmissiveNode(cfg: EffectConfig) {
 ...
 */
function buildBurningEmissiveNode(cfg: EffectConfig) {
 ...
 */
function buildFrozenEmissiveNode(cfg: EffectConfig) {
...
const BUILDERS: Record<OverlayEffectId, (cfg: EffectConfig) => ReturnType<typeof buildWorleyEmissiveNode>> = {
...
export function useStatusEffectOverlay(
  ...
  const activeEffect = shallowRef<OverlayEffectId | null>(null)
  ...
  function applyOverlay(effectId: OverlayEffectId) {
    clearOverlay()
    if (!rig.value) return
    const emissiveNode = BUILDERS[effectId](OVERLAY_EFFECTS[effectId])
  ...
  watch(
    ...
    ({ effects }) => {
      const next = PRIORITY.find(e => effects?.some((se: { id: StatusEffectId }) => se.id === e)) ?? null
