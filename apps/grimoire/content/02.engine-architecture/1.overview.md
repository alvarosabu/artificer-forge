...
The game runtime lives in its own package, `@artificer-forge/engine`. Apps (`playground`, `game`) stay thin: they own content, routes and app-level config, while the engine owns the rules, the runtime state, the scene host and the HUD. This is the codebase's stated philosophy in practice — **prototype in the playground, stabilize, extract to the engine package**.
