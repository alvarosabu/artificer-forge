...
Usage:
```ts
gameStore.equipWeapon(rangerId, 'shortsword', 'mainHand')
gameStore.unequipWeapon(rangerId, 'mainHand')
```
...
gameStore.selectEntity(rangerId)
gameStore.equipWeapon(rangerId, 'shortsword', 'mainHand')

// Load scene - spawns world entities, returns spawn point
const { spawnPoint } = await gameStore.loadScene('goblin_hideout')
gameStore.updateEntity(rangerId, { position: spawnPoint })

// Combat: deal damage
const entity = gameStore.getEntity(rangerId)
if (entity?.hp) {
  gameStore.updateEntity(rangerId, { hp: entity.hp - 5 })
}
...
gameStore.updateEntity(rangerId, { position: newSpawn })
```
