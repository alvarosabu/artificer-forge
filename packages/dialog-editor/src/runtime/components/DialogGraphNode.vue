...
const props = defineProps<{
  id: string
  data: { nodeId: string, node: DialogNode, expanded?: boolean }
}>()
...
const expanded = computed(() => props.data.expanded ?? false)
</script>
...
<template>
  <div class="de-node" :class="{ expanded }">
    ...
    <header class="de-hd">
      ...
      <button class="de-chev" @click.stop="emit('toggle', id)">{{ expanded ? '▾' : '▸' }}</button>
...
.de-chev { background:none; border:none; color:#8b93a7; cursor:pointer; font-size:13px; }
...
.de-footer { padding:7px 10px; border-top:1px solid #2a3040; color:#8b93a7; position:relative; }
